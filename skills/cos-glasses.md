...
- **Model → effort:** Scroll highlights a row; one tap selects it. `>` is the cursor, `*` is the current setting. A model tap saves that model immediately and opens effort; an effort tap applies the level and returns to the page where you started. Tap the already-starred model to change effort only. Lens labels `X-High` and `Ultra` correspond to Extra High and Ultracode. Scroll up beyond the first row to leave; leaving effort does not undo an already-chosen model. Settings apply to your next message, not an in-flight answer. [Practice the model and effort picker](https://www.gotcos.com/docs/#models-ring).
...
- [Ask COS](https://www.gotcos.com/docs/#ask-ring): starting dictation is not sending. Tap to finish; the listening view becomes the transcribed prompt with `Tap=Send` in the footer. Review it before tapping. Scroll down from the final transcript page for Re-record, Cancel, Send original, Edit, Preview, and Change Model. Scrolling highlights; a separate tap selects. Double-tap during capture or review leaves the draft intact. During an active run, cancellation normally takes two double-taps: arm, then confirm within three seconds. Read the footer; a watched job can explicitly advertise a ready reply instead.
