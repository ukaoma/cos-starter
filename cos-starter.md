...
## Steps
1. On first setup, run `npx --yes @gotcos/glasses-server@latest --setup-transcription` from this COS folder. It provisions Small.en provisional preview, Large-v3-Turbo committed live text, and Large-v3 HQ polish, then starts the server with this COS brain. On later starts, use the normal command. The server prints the URL + API token and saves the token to `~/.cos-glasses/.env`.
